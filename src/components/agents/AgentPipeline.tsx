        ...
        <div className="flex gap-2">
          <button onClick={onToggleAuto}
            className={`text-[11px] font-mono px-3 py-1.5 rounded-lg border transition-all ${
              autoMode ? 'bg-mint/10 border-mint/30 text-mint' : 'bg-navy-600 border-steel/20 text-muted hover:text-offwhite'
            }`}>
            {autoMode ? '⏸ Stop Auto' : '⟳ Auto'}
          </button>
          <button onClick={onRun} disabled={running}
            className={`text-[11px] font-mono px-4 py-1.5 rounded-lg border transition-all font-medium ${
              running ? 'bg-gold/10 border-gold/20 text-gold/60 cursor-not-allowed' : 'bg-gold/15 border-gold/40 text-gold-light hover:bg-gold/25'
            }`}>
          ...
          return (
            <div key={agent.name} className="flex gap-3">
              <div className="flex flex-col items-center" style={{ width: 20 }}>
                <div className="w-2.5 h-2.5 rounded-full mt-3 flex-shrink-0 transition-all duration-300"
                  style={{
              ...
              <div className="flex-1 pb-3">
                <div className="flex items-center gap-2 mb-0.5">
                  ...
                  {isRunning && <span className="badge badge-gold text-[9px]">RUNNING</span>}
